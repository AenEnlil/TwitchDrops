...
from bot.keyboard import menu
from bot.text import new_campaigns_text
...
async def notify_users_about_new_campaign(user_ids: list, campaigns: list):
    ...
    for user_id in user_ids:
        messages = form_response_message(campaigns)

        try:
            for message in messages:
                await bot.send_message(chat_id=user_id, text=new_campaigns_text+message, parse_mode=ParseMode.HTML)
        except TelegramBadRequest as e:
            await bot.send_message(chat_id=user_id, text=f'Error: {e.message}', reply_markup=menu)
